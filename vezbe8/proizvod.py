...
class Racun:
    ...
    def sadrzi(self, proizvod):
        return proizvod in self.__proizvodi
...
class Podaci:
    ...
    def pronadji_proizvod(self, sifra):
    ...
    def obrisi_proizvod(self, indeks):
        proizvod = self.__proizvodi.pop(indeks)

        # obriši sve račune koji sadrže proizvod koji se briše
        racuni_za_brisanje = []
        for racun in self.__racuni:
            if racun.sadrzi(proizvod):
                racuni_za_brisanje.append(racun)
        for racun in racuni_za_brisanje:
            self.__racuni.remove(racun)
